...
namespace kdab {
extern void FlutterDesktopMessengerSendResponse(FlutterDesktopMessengerRef messenger,
    ...
    const std::function<void(void)> &input_unblock_cb)
{
    ...
    if (callback_iterator == callbacks_.end()) {
        kdab::FlutterDesktopMessengerSendResponse(messenger_, message.response_handle,
                                                  nullptr, 0);
        return;
    }
